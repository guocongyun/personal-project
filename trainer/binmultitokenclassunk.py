...
#             preds["input_ids"] += input_ids.cpu().tolist()
        ...
        for idx, batch in enumerate(tqdm(valid_dataloader)):
            with torch.no_grad():
                ...
                token_labels = token_labels[zero_msk&unk_mask]
                token_valid = [label2target[str(l)] for l in mult_labels[zero_msk&unk_mask].cpu().tolist()]
                mult_labels = torch.where(unk_mask, torch.tensor(0), mult_labels)
                
                if token_labels.sum() == 0: continue
                bin_output, mult_output, token_output = model(input_ids=input_ids, attention_mask=att_mask, zero_msk=zero_msk&unk_mask)
                ...
                preds["token_valid"] += get_keyphrase(input_ids[zero_msk&unk_mask], token_output)
