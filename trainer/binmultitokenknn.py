...
def trainer(epochs=20,
    ...
    for epoch in range(epochs):
        ...
        for idx, batch in enumerate(tqdm(train_dataloader)):
            ...
            bin_outputs += bin_output
            mult_outputs += mult_output
            hss += hs.cpu().tolist()
...
#         print(np.shape(list(hs2bin.keys())[0]))
        hss = torch.tensor(hss).to(device)
        model.eval()
        for idx, batch in enumerate(tqdm(valid_dataloader)):
            with torch.no_grad():
                ...
                _, top_k_indices = torch.topk(cos_sim, 4, dim=-1)
                knn = hss[top_k_indices.view(-1)].view(bs, 4, -1)
                bin_knn = torch.zeros(bs, 2).to(device)
                mult_knn = torch.zeros(bs, 500).to(device)
                for i in range(knn.shape[0]):
                    for j in range(knn.shape[1]):
                        key = tuple(knn[i, j, :].tolist())
                        bin_value, mult_value = hs2bin[key], hs2mult[key]
#                         bin_value = hs2bin.get(key, float('nan'))  
#                         mult_value = hs2mult.get(key, float('nan'))
#                         print(type(bin_value))
#                         print(type(mult_value))
                        bin_knn[i] += bin_value * 0.05
                        mult_knn[i] += mult_value * 0.05
#                 print(type(bin_output))
#                 print(type(bin_knn))
#                 print(type(mult_output))
#                 print(type(mult_knn))
                preds["bin_valid"] += torch.argmax(bin_output*0.8+bin_knn, axis=1).cpu().tolist()
                preds["mult_valid"] += torch.argmax(mult_output*0.8+mult_knn, axis=1).cpu().tolist()
