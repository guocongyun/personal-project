...
def trainer(epochs=20,
    ...
    ids2binl = dict(zip(map(tuple, dataset["test"]["input_ids"]), torch.zeros(len(dataset["test"]["input_ids"]))))
    ids2multl = dict(zip(map(tuple, dataset["test"]["input_ids"]), torch.zeros(len(dataset["test"]["input_ids"]))))
    ids2tokenl = dict(zip(map(tuple, dataset["test"]["input_ids"]), torch.zeros(len(dataset["test"]["input_ids"]))))
    delta = 0.3
    for epoch in range(epochs):
        ...
        for idx, batch in enumerate(tqdm(train_dataloader)):
            ...
            l = len(input_ids)
            bin_l = torch.tensor([0.0]).to(device)
            mult_l = torch.tensor([0.0]).to(device)
            for ids in input_ids.cpu().tolist():
                bin_l += ids2binl[tuple(ids)] * delta / l
                mult_l += ids2multl[tuple(ids)] * delta / l
                
            tl = len(input_ids[zero_msk])
            token_l = torch.tensor([0.0]).to(device)
            for ids in input_ids[zero_msk].cpu().tolist():
                token_l += ids2tokenl[tuple(ids)] * delta / tl
            
            bin_output, mult_output, token_output = model(input_ids=input_ids, attention_mask=att_mask, zero_msk=zero_msk)
            bin_loss = loss_func(bin_output, bin_labels) + bin_l
            mult_loss = loss_func(mult_output, mult_labels) + mult_l
            token_loss = loss_func(token_output.view(-1, 2), token_labels.view(-1)) + token_l
            ...
            for ids in input_ids.cpu().tolist():
                ids_tuple = tuple(ids)
                ids2binl[ids_tuple] = bin_loss.detach()
                ids2multl[ids_tuple] = mult_loss.detach()
                ids2tokenl[ids_tuple] = token_loss.detach()
