...
from models.distillbert import DistillBertMult
from utils import *
...
def trainer(epochs=20,
    ...
    if hidden_act not in ("geglu", "reglu"):
        ...
        model = DistillBertMult(config=configuration)
    else: model = DistillBertMult(config=configuration, custom_hidden_act=hidden_act)
...
#             if (idx+1) % int(128/batch_size) == 0:
            ...
            preds["train"] += torch.argmax(output, axis=1).cpu().tolist()
        ...
        for idx, batch in enumerate(tqdm(valid_dataloader)):
            with torch.no_grad():
                ...
                # Move tensors back to cpu for sklearn
                preds["valid"] += torch.argmax(output, axis=1).cpu().tolist()
                truth["valid"] += labels.cpu().tolist()
            
        train_f1 = f1_score(truth["train"], preds["train"], average='weighted')  
        valid_f1 = f1_score(truth["valid"], preds["valid"], average='weighted')  
        print('F1 Score:', train_f1)
        print('F1 Score:', valid_f1)
        train_f1 = f1_score(truth["train"], preds["train"], average='micro')  
        valid_f1 = f1_score(truth["valid"], preds["valid"], average='micro')  
        print('F1 Score:', train_f1)
        print('F1 Score:', valid_f1)
