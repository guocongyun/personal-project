...
from models.distillbert import DistilBertForBinMultTokenLoss
...
def trainer(epochs=20,
    ...
    if hidden_act not in ("geglu", "reglu"):
        ...
        model = DistilBertForBinMultTokenLoss(config=configuration)
    else: model = DistilBertForBinMultTokenLoss(config=configuration, custom_hidden_act=hidden_act)
    ...
    for epoch in range(epochs):
        ...
        for idx, batch in enumerate(tqdm(train_dataloader)):
            ...
            bin_output, mult_output, token_output, loss_weight = model(input_ids=input_ids, attention_mask=att_mask, zero_msk=zero_msk)
            loss_weight = torch.nn.functional.softmax(loss_weight, dim=0)
            ...
            combined_loss = loss_weight[0]*bin_loss+loss_weight[1]*mult_loss+loss_weight[2]*token_loss
        ...
        for idx, batch in enumerate(tqdm(valid_dataloader)):
            with torch.no_grad():
                ...
                bin_output, mult_output, token_output, _ = model(input_ids=input_ids, attention_mask=att_mask, zero_msk=zero_msk)
