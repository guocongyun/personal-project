from transformers import BertModel, BertPreTrainedModel, AutoModelForMaskedLM, AutoModel
...
class BertForPretrain(BertPreTrainedModel):
    def __init__(self, config, custom_hidden_act=False, pretrained_path="bert-base-uncased"):
        super().__init__(config)

        # Bert base model
        self.bert = AutoModelForMaskedLM.from_pretrained(pretrained_path, config=config)
        self.hidden_act = nn.ReLU()
        if custom_hidden_act == "geglu":
            for i in range(len(self.bert.encoder.layer)):
                self.bert.encoder.layer[i].intermediate.act_fn = GEGLU()
                self.hidden_act = GEGLU()
        elif custom_hidden_act == "reglu":
            for i in range(len(self.bert.encoder.layer)):
                self.bert.encoder.layer[i].intermediate.act_fn = ReGLU()
                self.hidden_act = ReGLU()
#         self.bert = AutoModel.from_pretrained(config=config)
    ...
    def forward(
        ...
        hidden_states = bert_output[0]  # (batch_size, sequence_length, dim)
...
class DistillBertMult(torch.nn.Module):
    def __init__(self, config, custom_hidden_act=False, pretrained_path="bert-base-uncased"):
        ...
        self.classifier = torch.nn.Linear(768, 455)
...
class BertForToken(torch.nn.Module):
    def __init__(self, config, custom_hidden_act=False, pretrained_path="bert-base-uncased"):
        super().__init__()
        self.bert = BertModel.from_pretrained(pretrained_path, config=config)
        self.hidden_act = nn.ReLU()
        if custom_hidden_act == "geglu":
            ...
            for i in range(len(self.bert.encoder.layer)):
                ...
                self.hidden_act = ReGLU()
        self.dropout = torch.nn.Dropout(config.dropout)
        self.classifier = torch.nn.Linear(self.bert.config.dim, 2)
...
class BertForBinMult(torch.nn.Module):
    def __init__(self, config, custom_hidden_act=False, pretrained_path="bert-base-uncased"):
        super().__init__()
        self.bert = BertModel.from_pretrained(pretrained_path, config=config)
        self.hidden_act = nn.ReLU()
...
class BertForBinMultToken(torch.nn.Module):
    def __init__(self, config, custom_hidden_act=False, pretrained_path="bert-base-uncased"):
        super().__init__()
        ...
        self.token_classifier = torch.nn.Sequential(
            self.dropout,
            torch.nn.Linear(self.bert.config.dim, 2)
        )
...
class BertForBinMultTokenFreeze(torch.nn.Module):
    def __init__(self, config, custom_hidden_act=False, pretrained_path="bert-base-uncased"):
        super().__init__()
        self.bert = BertModel.from_pretrained(pretrained_path, config=config)
        self.hidden_act = nn.ReLU()
...
class BertForBinMultTokenKNN(torch.nn.Module):
    def __init__(self, config, custom_hidden_act=False, pretrained_path="bert-base-uncased"):
        super().__init__()
        self.bert = BertModel.from_pretrained(pretrained_path, config=config)
        self.hidden_act = nn.ReLU()
...
class BertForBinMultTokenLearnLoss(torch.nn.Module):
    def __init__(self, config, custom_hidden_act=False, pretrained_path="bert-base-uncased"):
        super().__init__()
        self.bert = BertModel.from_pretrained(pretrained_path, config=config)
        self.hidden_act = nn.ReLU()
...
class BertForBinMultTokenLoss(torch.nn.Module):
    def __init__(self, config, custom_hidden_act=False, loss = "avr", pretrained_path="bert-base-uncased"):
        super().__init__()
        self.bert = BertModel.from_pretrained(pretrained_path, config=config)
        self.hidden_act = nn.ReLU()
        if custom_hidden_act == "geglu":
...
#         self.bin_classifier = torch.nn.Linear(self.bert.config.dim, 2)
